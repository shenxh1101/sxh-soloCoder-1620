...
def create_rental(customer_id, equipment_id, start_date, expected_return_date,
    ...
    if equipment['status'] == '在租':
        return False, None, f"设备 {equipment['code']} 当前已在租"
...
def calculate_rental_fee(rental_id, actual_return_date, return_hours):
    ...
    if rental['rental_mode'] == '按天':
        rental_days = _days_between(rental['start_date'], actual_return_date)
        expected_days = _days_between(rental['start_date'], rental['expected_return_date'])
        base_rent = rental_days * rental['daily_rate']
        overtime_days = max(0, rental_days - expected_days)
        overtime_fine = overtime_days * rental['daily_rate'] * 1.5
    else:
        used_hours = return_hours - rental['start_hours']
        expected_days = _days_between(rental['start_date'], rental['expected_return_date'])
        expected_hours = expected_days * 8
        base_rent = used_hours * rental['hourly_rate']
        overtime_hours = max(0, used_hours - expected_hours)
        overtime_fine = overtime_hours * rental['hourly_rate'] * 1.5
    ...
    return {
        ...
        'used_hours': round(return_hours - rental['start_hours'], 2),
        'rental_days': _days_between(rental['start_date'], actual_return_date)
    }
...
def return_equipment(rental_id, actual_return_date, return_hours):
    ...
    try:
        ...
        cursor.execute('UPDATE equipment SET status = ?, total_hours = ? WHERE id = ?',
                       ('空闲', return_hours, rental['equipment_id']))
        conn.commit()
        return True, fees
    except Exception as e:
        return False, str(e)
